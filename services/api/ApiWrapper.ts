...
import { PROJECTS_MOCK, USER_MOCK } from "@/mockData";
...
// API REQUEST FUNCTION WITH BUILT-IN ERROR HANDLING, AUTH, HMAC SIGNATURE SUPPORT, AND URL PARAM/QUERY RESOLUTION.
// Usage:
// 1. Define your endpoint in ApiEndpoint.ts with the appropriate config.
// 2. Call apiRequest with the endpoint key and any necessary body or options.
// 3. The function handles auth headers, HMAC signatures, error parsing, and returns typed data.
...
export async function apiRequest<T>(
  ...
  // ==================== MOCK BYPASS LOGIC ====================
  // If we're in development and don't have a backend, return mock data immediately
  logger.info("apiRequest: MOCK MODE ACTIVE", { endpointKey });

  // Artificial delay to simulate network latency
  await new Promise((resolve) => setTimeout(resolve, 500));

  if (endpointKey === "dashboard-stats" || endpointKey === "sample") {
    return PROJECTS_MOCK as unknown as T;
  }
  
  if (endpointKey === "permissions") {
    return { permissions: ["READ", "WRITE", "ADMIN"] } as unknown as T;
  }

  // Fallback to USER_MOCK if nothing else matches
  return USER_MOCK as unknown as T;
  // ===========================================================
}